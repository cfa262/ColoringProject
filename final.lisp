...
;; takes a list of coloring assignments (solution) and the graph
;; and verifies that it is a valid solution
;; Author: Calvin Alvarez
(defun test-solution (lst graph)
	(dolist (x lst)
		(setf color (cdr x)) ;;sets color to be checked
		(setf element (list (car x)))c;;sets element to find
		...
		(dolist (y graph) ;;loops through graph
			...
			(cond ((equal element (list (car y)))
				(setf el (car (cdr y))) ;;gets the list of edges
					...
					;;loops through list of edges then the
					;;solved solution list
